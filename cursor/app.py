...
SMS_API_CONFIG = {
    'API_URL': os.environ.get('SMS_API_URL', 'https://yurticisms1.com/index'),
...
def send_sms_batch(numbers, message):
    ...
    # Prepare parameters - using exact parameter names that work with the API
    params = {
    ...
    try:
        ...
        if "error" in response_text.lower():
            return {
                "Status": "Error",
                "Message": f"API Hatası: {response_text}"
            }
            
        # Check for success
        if "ok" in response_text.lower() or response_text.startswith("00"):
            return {
                "Status": "Success",
        ...
        return {
            "Status": "Error",
            "Message": f"API Yanıtı: {response_text}"
        }
